...
const TimeSelection = ({ selectedMovie, theaterNo, selectedDateString, selectedTime, selectedHall, setSelectedTime, setSelectedHall }) => {
    ...
    const handleTimeClick = (time, hall) => {
        ...
        setSelectedHall(hall.theaterNo);
    };

    if (!selectedMovie || !theaterNo || !selectedDateString) {
        return null; // 조건이 충족되지 않으면 아무것도 렌더링 x
    }
